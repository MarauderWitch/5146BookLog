 ...
 */class cY{constructor(e){this.apiKey=e}getGenerativeModel(e,t){if(!e.model)throw new cw("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new cQ(this.apiKey,e,t)}getGenerativeModelFromCachedContent(e,t,n){if(!e.name)throw new cb("Cached content must contain a `name` field.");if(!e.model)throw new cb("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==t?void 0:t[n])&&e[n]&&(null==t?void 0:t[n])!==e[n]){if("model"===n&&(t.model.startsWith("models/")?t.model.replace("models/",""):t.model)===(e.model.startsWith("models/")?e.model.replace("models/",""):e.model))continue;throw new cb(`Different value for "${n}" specified in modelParams (${t[n]}) and cachedContent (${e[n]})`)}let r=Object.assign(Object.assign({},t),{model:e.model,tools:e.tools,toolConfig:e.toolConfig,systemInstruction:e.systemInstruction,cachedContent:e});return new cQ(this.apiKey,r,n)}}document.addEventListener("DOMContentLoaded",function(){o(cy).setLevel("info"),o(cy).info("Application started"),console.log("Firebase Firestore:",r,i);let e=new he,t=document.getElementById("signIn");t?t.addEventListener("click",function(t){hq(i,e).then(e=>{he.credentialFromResult(e).accessToken;let t=e.user;localStorage.setItem("email",JSON.stringify(t.email)),window.location="dashboard.html"}).catch(e=>{e.code,e.message,e.customData.email,he.credentialFromError(e)})}):console.error("Sign-in button not found."),console.log("Email:",localStorage.getItem("email")?JSON.parse(localStorage.getItem("email")):null);let n=document.getElementById("logout");if(n?n.addEventListener("click",function(e){localStorage.removeItem("email"),window.location.href="index.html"}):console.error("Logout button not found."),console.log("Checking for book-form:",document.getElementById("book-form")),document.getElementById("book-form")){console.log("Initializing Book Logic..."),document.getElementById("book-form");let e=document.getElementById("book-list");async function s(e,t,n,s){i.onAuthStateChanged(async i=>{if(!i){console.error("No user signed in. Cannot add book.");return}try{await function(e,t){var n;let r=a$(e.firestore,aZ),i=aY(e),s=(n=e.converter)?n.toFirestore(t):t;return lk(r,[(function(e,t,n,r,i,s={}){let o,a;let l=e.ju(s.merge||s.mergeFields?2:0,t,n,i);ls("Data must be an object, but it was:",l,r);let u=lr(r,l);if(s.merge)o=new n7(l.fieldMask),a=l.fieldTransforms;else if(s.mergeFields){let e=[];for(let r of s.mergeFields){let i=function(e,t,n){if((t=eq(t))instanceof a2)return t._internalPath;if("string"==typeof t)return la(e,t);throw ll("Field path arguments must be of type string or ",e,!1,void 0,n)}(t,r,n);if(!l.contains(i))throw new nS(nC.INVALID_ARGUMENT,`Field '${i}' is specified in your field mask but missing from your input data.`);(function(e,t){return e.some(e=>e.isEqual(t))})(e,i)||e.push(i)}o=new n7(e),a=l.fieldTransforms.filter(e=>o.covers(e.field))}else o=null,a=l.fieldTransforms;return new a4(new rM(u),o,a)})(lt(e.firestore),"addDoc",i._key,s,null!==e.converter,{}).toMutation(i._key,iD.exists(!1))]).then(()=>i)}(aQ(r,"books"),{title:e,author:t,genre:n,rating:s,email:i.email}),console.log(`Book "${e}" added successfully by ${i.email}.`),a()}catch(e){console.error("Error adding book:",e)}})}async function a(){console.log("Fetching books..."),i.onAuthStateChanged(async t=>{if(!t){console.log("No user signed in. Cannot fetch books.");return}try{let n=lp(aQ(r,"books"),lm("email","==",t.email)),i=await lA(n);if(e.innerHTML="",i.empty){e.innerHTML="<p>No books added yet.</p>",console.log("No books found in database.");return}i.forEach(t=>{let n=t.data(),r=t.id,i=document.createElement("li");i.tabIndex=0,i.id=r,i.innerHTML=`
                        ...
                        `,e.appendChild(i)}),console.log("Books successfully displayed.")}catch(e){console.error("Error fetching and displaying books:",e)}})}async function l(t,n){console.log(`Filtering books where ${t} == ${n}`),i.onAuthStateChanged(async i=>{if(!i){console.log("No user signed in. Cannot filter books.");return}try{let s=lp(aQ(r,"books"),lm("email","==",i.email),lm(t,"==",n)),o=await lA(s);if(e.innerHTML="",o.empty){e.innerHTML=`<p>No books found for ${t} "${n}".</p>`,console.log(`No books found for ${t} "${n}".`);return}o.forEach(t=>{let n=t.data(),r=t.id,i=document.createElement("li");i.tabIndex=0,i.id=r,i.innerHTML=`
                            <strong>${n.title} </strong>
                            by <span class="author-tag" data-author="${n.author}">${n.author}</span>
                            <em> (<span class="genre-tag" data-genre="${n.genre}">${n.genre}</span>) </em>
                            - Rating: ${n.rating}
                            <button class="delete-btn" data-id="${r}">Delete</button>
                        `,e.appendChild(i)}),console.log("Filtered books successfully displayed.")}catch(e){console.error("Error fetching and displaying filtered books:",e)}})}if(document.addEventListener("click",async e=>{if(e.target.classList.contains("author-tag")){let t=e.target.getAttribute("data-author");console.log(`Filtering books by author: ${t}`),l("author",t)}if(e.target.classList.contains("genre-tag")){let t=e.target.getAttribute("data-genre");console.log(`Filtering books by genre: ${t}`),l("genre",t)}}),document.addEventListener("click",async e=>{if(e.target.classList.contains("delete-btn")){let n=e.target.getAttribute("data-id");try{var t;await (t=aY(r,"books",n),lk(a$(t.firestore,aZ),[new iH(t._key,iD.none())])),console.log(`Book with ID ${n} deleted.`),a()}catch(e){console.error("Error deleting book:",e)}}}),document.getElementById("book-form")){console.log("Book form found. Initializing book logic...");let e=document.getElementById("book-form");document.getElementById("book-list"),e.addEventListener("submit",function(t){console.log("Form submit event detected."),t.preventDefault(),console.log("Book form submitted.");let n=document.getElementById("title").value.trim(),r=document.getElementById("author").value.trim(),i=document.getElementById("genre").value.trim(),o=document.getElementById("rating").value.trim();if(!n||!r||!i||!o){console.error("All fields must be filled.");return}console.log("Form values:",{title:n,author:r,genre:i,rating:o}),s(n,r,i,o),e.reset()}),a()}else console.error("Book form not found.");document.getElementById("reset-filter").addEventListener("click",()=>{console.log("Resetting book list filter..."),a()})}let u=document.getElementById("chat-input"),h=document.getElementById("send-btn"),c=document.getElementById("chat-history"),f=document.getElementById("chatbot-container");async function d(){model=(genAI=new cY(apiKey=(await function(e){e=a$(e,aG);let t=a$(e.firestore,aZ);return(function(e,t,n={}){let r=new nA;return e.asyncQueue.enqueueAndForget(async()=>(function(e,t,n,r,i){let s=new ak({next:a=>{s.su(),t.enqueueAndForget(()=>o1(e,o));let l=a.docs.has(n);!l&&a.fromCache?i.reject(new nS(nC.UNAVAILABLE,"Failed to get document because the client is offline.")):l&&a.fromCache&&r&&"server"===r.source?i.reject(new nS(nC.UNAVAILABLE,'Failed to get document from server. (However, this document does exist in the local cache. Run again without setting source to "server" to retrieve the cached document.)')):i.resolve(a)},error:e=>i.reject(e)}),o=new o5(new r8(n.path),s,{includeMetadataChanges:!0,Ta:!0});return o0(e,o)})(await aL(e),e.asyncQueue,t,n,r)),r.promise})(a0(t),e._key).then(n=>(function(e,t,n){let r=n.docs.get(t._key),i=new lS(e);return new lI(e,i,t._key,r,new lb(n.hasPendingWrites,n.fromCache),t.converter)})(t,e,n))}(aY(r,"apikey","googlegenai"))).data().key)).getGenerativeModel({model:"gemini-1.5-flash"})}async function p(e){return(await model.generateContent(e)).candidates[0].content.parts[0].text}function g(e){let t=document.createElement("div");t.textContent=e,t.className="history",c.appendChild(t)}document.getElementById("chat-icon"),f.addEventListener("click",function(e){f.classList.contains("expanded")||(f.classList.add("expanded"),e.stopPropagation())}),document.addEventListener("click",function(e){!f.contains(e.target)&&f.classList.contains("expanded")&&f.classList.remove("expanded")}),c.addEventListener("click",e=>e.stopPropagation()),u.addEventListener("click",e=>e.stopPropagation()),h.addEventListener("click",e=>e.stopPropagation()),u&&h&&h.addEventListener("click",async()=>{let e=u.value.trim();if(e){var t;if(g(`You: ${e}`,"user"),!((t=(t=e).toLowerCase().trim()).startsWith("add")?(g("To add a book, enter title, author, genre, and rating, then click 'Add Book'."),!0):t.startsWith("delete")?(g("To delete a book, click the 'Delete' button next to it."),!0):t.startsWith("sort")?(g("To sort through books, click either on the author's name or on the book's genre."),!0):(g("I'm sorry, I didn't understand that command."),!1)))try{let t=await p(e);g(`Chatbot: ${t}`,"bot")}catch(e){console.error("Error with AI chatbot:",e),g("Chatbot: Sorry, I couldn't process that request.","bot")}u.value=""}}),!async function(){await d(),console.log("AI initialized successfully.")}()});
//# sourceMappingURL=dashboard.13dbb9e9.js.map
