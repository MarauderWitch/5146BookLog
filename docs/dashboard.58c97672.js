 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class eu{constructor(e){this.apiKey=e}getGenerativeModel(e,t){if(!e.model)throw new P("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new eh(this.apiKey,e,t)}getGenerativeModelFromCachedContent(e,t,n){if(!e.name)throw new M("Cached content must contain a `name` field.");if(!e.model)throw new M("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==t?void 0:t[n])&&e[n]&&(null==t?void 0:t[n])!==e[n]){if("model"===n&&(t.model.startsWith("models/")?t.model.replace("models/",""):t.model)===(e.model.startsWith("models/")?e.model.replace("models/",""):e.model))continue;throw new M(`Different value for "${n}" specified in modelParams (${t[n]}) and cachedContent (${e[n]})`)}let r=Object.assign(Object.assign({},t),{model:e.model,tools:e.tools,toolConfig:e.toolConfig,systemInstruction:e.systemInstruction,cachedContent:e});return new eh(this.apiKey,r,n)}}if("undefined"==typeof firebase)console.error("Firebase SDK not loaded correctly.");else{let e=new N.X;function ec(){(0,x.auth).signOut().then(()=>{console.log("User signed out."),localStorage.removeItem("email"),window.location="index.html"}).catch(e=>{console.error("Logout error:",e)})}document.getElementById("signIn").addEventListener("click",function(t){x.auth,(0,N.c)(x.auth,e).then(e=>{(0,N.X).credentialFromResult(e).accessToken;let t=e.user;localStorage.setItem("email",JSON.stringify(t.email)),window.location="dashboard.html"}).catch(e=>{e.code,e.message,e.customData.email,(0,N.X).credentialFromError(e)})}),document.addEventListener("DOMContentLoaded",()=>{let e=document.getElementById("chat-input"),t=document.getElementById("send-btn"),n=document.getElementById("chat-history"),r=document.getElementById("logout");if(r&&r.addEventListener("click",ec),document.getElementById("book-form")){let r=document.getElementById("book-form"),h=document.getElementById("book-list");function i(){console.log("Fetching books..."),(0,x.db).collection("books").get().then(e=>{if(e.empty){h.innerHTML="<p>No books added yet.</p>",console.log("⚠ No books in database.");return}e.forEach(e=>{let t=e.data(),n=e.id;console.log("Found book:",t),function(e,t){let n=document.createElement("li");n.innerHTML=`
...
//# sourceMappingURL=dashboard.58c97672.js.map
